...
let
  ...
  # Mapped to the structure found in the AnkiRecolor source/meta.json
  recolorConfig = {
    colors = {
      # Main Backgrounds
  ...
  # 3. Anki Recolor (Manually patched with Tokyo Night via meta.json)
  anki-recolor = pkgs.stdenv.mkDerivation {
    ...
    # We overwrite meta.json which contains the internal addon config
    installPhase = ''
      mkdir -p $out
      cp -r * $out/
